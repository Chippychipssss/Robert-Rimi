...
var createBook = (book, callback) => {
    ...
    db.executeQuery(sql, [0, book.grade, book.title, book.author, book.publisher, book.edition, book.isbn, book.pages, date], function(result) {
        callback(result);
    });
};
...
var updateBook = (id, book, callback) => {
    var sql = "UPDATE books SET grade = ?, title = ?, author = ?, publisher = ?, edition = ?, isbn = ?, pages = ? WHERE book_id = ?";
    db.executeQuery(sql, [book.grade, book.title, book.author, book.publisher, book.edition, book.isbn, book.pages, id], function(result) {
        callback(result);
    });
};
...
var bookRequest = (customer_id, book, callback) => {
    ...
    db.executeQuery(sql, [customer_id, book.grade, book.title, book.author, book.edition, book.isbn, date], function(result) {
        callback(result);
    });
};
